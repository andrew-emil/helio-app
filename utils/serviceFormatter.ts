...
export function formatServicesByCategory(services: FormattedService[]): ServiceCategory[] {
    ...
    const formattedCategories: ServiceCategory[] = Object.entries(categoriesMap).map(([categoryName, subCategoriesMap]) => {
        const subCategories: ServiceSubCategory[] = Object.entries(subCategoriesMap).map(([subCategoryName, services]) => ({
            subCategoryName,
            services: services.sort((a, b) => a.name.localeCompare(b.name)) // Optional: sort services by name
        }));

        return {
            categoryName,
            subCategories: subCategories.sort((a, b) => a.subCategoryName.localeCompare(b.subCategoryName)) // Optional: sort subcategories
        };
    });

    return formattedCategories.sort((a, b) => a.categoryName.localeCompare(b.categoryName)); // Optional: sort categories
}